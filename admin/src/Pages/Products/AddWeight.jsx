...
const AddWeight = () => {
    ...
    const[name,setName]=useState()
        ...
        const handleSubmit=(e)=>{
          ...
          setIsLoading(true);
              ...
              if(editMode == ""){
                ...
                if(res?.success){
                   Context.alertBox( "success",  res?.message || "Created Product Weight  successfully!" );
                    setIsLoading(false)
                    getData();
                    setName('')
                } else {
                ...
                editData(`/api/product/updateProductWeight/${editMode}`,{
                    name:name
              }).then((res)=>{  
                if(res?.success){
                   Context.alertBox( "success",  res?.message || "Created Product Weight  successfully!" );
        ...
        const deleteWeight=(id)=>{
            deleteData(`/api/product/deleteProductWeight/${id}`).then((res)=>{
                getData();
                Context.alertBox("Success","Product Weight deleted")
                
            })
        }
    
        const editItem=(id)=>{
            fetchDataFromApi(`/api/product/getProductsWeight/${id}`).then((res)=>{  
  ...
  return (
    <>
      <div className="flex items-center justify-between px-2 py-0 mt-3">
          <h2 className="text-[18px] font-[700]">Add Products Weight</h2>
        </div>
