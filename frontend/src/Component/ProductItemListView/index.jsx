    ...
    <>
       <div className="productItem shadow-md bg-[#f1f1f1] rounded-md overflow-hidden border-1 border-[rgba(0,0,0,0.1)] flex items-center">
        <div className="group imgWrapper w-[25%] overFlow-hidden rounded-md relative">
            ...
            <Link to={`/productDetails/${props?.item?._id}`}>
               <div className="img h-[220px] overflow-hidden">
                 ...
                 className=' w-full transition-all duration-700 absolute top-0 left-0 opacity-0 group-hover:opacity-100 group-hover:scale-100'/>
               </div>
            </Link>
            ...
            flex-col w-[50px] transition-all duration-300 group-hover:top-[15px] opacity-0 group-hover:opacity-100">
              ...
              <Button sx={{  minWidth: 30,  width: 30,  height: 30,  borderRadius: '50%',  backgroundColor: '#fff',  color: '#000',  '&:hover': {    backgroundColor: '#ff5252',    color: '#fff',  },}}
              onClick={()=>Context.setOpenProductDetailsModel(true, props?.item)}>
        ...
        <div className="info p-3 py-5   px-8 w-[75%] ">
           <h6 className='text-[20px] text-left title mt-2 mb-3 font-[500] text-[rgba(0,0,0,0.9)]'><Link to={`/productDetails/${props?.item?._id}`} className='link transition-all'>{props?.item?.name}</Link></h6>
           <h5 className='text-[17px] mb-1 text-[rgba(77, 173, 89, 0.1)]'> <Link to={`/productDetails/${props?.item?._id}`} className='link transition-all'>In Stocks</Link></h5>

           <p className="text-[14px] font-[400] mb-3">{props?.item?.description?.substr(0,40+'...')}</p>
